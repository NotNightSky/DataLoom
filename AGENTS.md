...
- Backend abstraction lives in `src/lib/`: `types.ts` defines `DataDoc`, `GeneratorBackend` (now with a `version` field) and `GeneratorGroup`; `generators.ts` groups the `.loom` backends by id into `GROUPS`. Add new templates by dropping a `.loom` file in `src/lib/generators/` and importing it in `generators.ts`; `getGroup(id)` and `getBackend(id, version)` resolve them. `GeneratorGrid.tsx` renders one card per `GROUPS` entry via `CARD_ICONS` keyed by backend id (icon components are typed `any`).
...
- **Backend picker** (`src/app.tsx` + `GeneratorGrid.tsx`): the app starts with no group selected, so the picker page is shown until one is chosen; `App` stores a `GeneratorGroup` + selected `version`, derives the active `backend`, and passes both to `EditorPage` along with a back button that resets to the picker. Cards are rendered one per `GROUPS` entry with an icon (`CARD_ICONS` keyed by backend id, fallback `Blocks`), the backend `name`, `description`, per-version chips, and an "Open" CTA; the grid header shows the group count.
- **Backend contract**: a backend is `{ id, name, version, description, defaultJson, generateJava(doc) }`. `defaultJson` seeds both the JSON editor text and the initial `DataDoc` (via `parseDefaultJson`, which falls back to `{ data: {} }`). Backends live as `.loom` files (YAML frontmatter + Handlebars template) compiled by the custom `vite-plugin-loom.ts`. **Versioning**: multiple `.loom` files may share one `id` with different `version` frontmatter (e.g. `recipe.loom` = `26.2`, `recipe_1_20.loom` = `1.20`); `src/lib/generators.ts` groups them by id into `GROUPS` (`GeneratorGroup`: `versions` sorted newest-first + `templatesByVersion`), with `getGroup(id)` and `getBackend(id, version)`. `EditorPage`'s toolbar has an MC version `<select>`; switching versions reseeds the editor with that backend's `defaultJson`. Custom Handlebars helpers (`toItemEnum`, `stripNamespace`, `capitalize`) are registered centrally in `src/lib/handlebars.ts`.
- **Editor workspace** (`EditorPage`): left pane is the JSON editor, right pane is read-only generated Java; both are Monaco (vs-dark). The splitter drags proportionally (`split` % clamped 18–82, default 61.5, splitter width 12px). Toolbar shows back button, `name — id`, the MC version `<select>`, and a spacer. Java is recomputed via `useMemo(generateJava(doc))` whenever the parsed doc or backend changes.
